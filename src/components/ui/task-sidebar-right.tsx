...
interface TaskSidebarRightProps {
	tasks: Task[];
	completedTasks?: Task[];
	sidebarTitle: string;
...
export function TaskSidebarRight({ 
	tasks, 
	completedTasks = [], 
	sidebarTitle, 
			...
			<ContextMenuTrigger asChild>
				...
				>
						...
						<div className='flex-1 min-w-0'>
							...
							<div className='flex items-center gap-2 mt-2 text-xs text-muted-foreground'>
								...
								<span className={cn(
									...
									task.status === 'today' && 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400',
									task.status === 'this-week' && 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400',
									task.status === 'backlog' && 'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-300'
								)}>
					...
					{isDraggable && !isCompleted && (
						<div className='text-xs text-muted-foreground mt-2 pt-2 border-t border-border opacity-0 group-hover:opacity-100 transition-opacity'>
							Drag to schedule or click to edit
						</div>
					)}
				</div>
			</ContextMenuTrigger>
	...
	return (
		<div className='w-80 border-l border-border bg-muted/20 p-4 overflow-y-auto flex flex-col'>
			{/* Main tasks section */}
			{tasks.length > 0 && (
				<div className='mb-6'>
